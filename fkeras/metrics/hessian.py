...
class HessianMetrics:
    ...
    def hessian_vector_product(self, v, layer_idx=None):
        ...
        with tf.GradientTape() as outer_tape:
            ...
            params = self.model.trainable_variables if layer_idx is None else self.model.trainable_variables[layer_idx]
    ...
    def trace(self, max_iter=100, tolerance=1e-3):
        ...
        for l_i in self.layer_indices:
            ...
            for i in range(max_iter):
                v = [np.random.uniform(shape=self.model.layers[l_i].trainable_variables[i].shape) for i in range(len(self.model.layers[l_i].trainable_variables))]
                ...
                # Compute the trace
                trace_vhv.append(tf.reduce_sum([tf.reduce_sum(vi * hvi) for (vi, hvi) in zip(v, hv)]))
        ...
        for l_i in self.layer_indices:
            ...
            for i in range(k):
                ...
                for j in range(max_iter):
                    ...
                    eigenvalue = tf.reduce_sum([tf.reduce_sum(vi * hvi) for (vi, hvi) in zip(v, hv)])
                    # Normalize the eigenvector
                    v = [hvi / tf.norm(hvi) for hvi in hv]
                    if abs(eigenvalue - eigenvalues[-1]) / (abs(eigenvalues[-1]) + 1e-6) < tolerance:
